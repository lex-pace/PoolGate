...
> 不改变：request\_logs（请求业务日志）、gateway\_logs（网关失败事件）现有逻辑；二者与文件日志互补，本方案不合并。
...
- 方案 B：在 `run()` 里用 `dirs::data_dir()`/`HOME` 推算路径（与 tauri 的 app\_data\_dir 可能不一致）——不推荐。
...
- 日志文件权限：随 app\_data\_dir（当前 `credentials.vault.json` 为 600，目录 700），落盘文件默认继承目录权限。
...
- **不做的事**：不合并 gateway\_logs / request\_logs；不做日志级别动态调整（后续可在设置页加级别选择，预留 `EnvFilter` 可替换）；不做远程日志。
