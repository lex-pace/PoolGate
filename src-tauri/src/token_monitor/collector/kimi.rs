//! Kimi CLI / Kimi Code（JSONL sessions / offset）— W3c。
//! 数据源：`~/.kimi/sessions/`、`~/.kimi-code/sessions/`。
...
jsonl_adapter! {
    ...
    dirs = {
        ...
        match home {
            Some(home) => vec![home.join(".kimi").join("sessions"), home.join(".kimi-code").join("sessions")],
            None => Vec::new(),
        }
    };
    ext = "jsonl";
